...
import { Entry, ParsedEntryByWeekRow } from "@/types/entries";

export default function WeeklyHistoryTable({
  entries,
}: {
  entries: ParsedEntryByWeekRow[];
}) {
  const [expandedWeek, setExpandedWeek] = useState("");
  const [expandedDay, setExpandedDay] = useState("");

  const toggleExpandedWeek = (key: string) => {
    setExpandedWeek(key === expandedWeek ? "" : key);
  };

  const toggleExpandedDay = (key: string) => {
    setExpandedDay(key === expandedDay ? "" : key);
  };
  ...
  return (
    <div className="border rounded-lg overflow-hidden">
      <table className="min-w-full text-white border border-white">
        ...
        <tbody>
          {entries?.map((week: ParsedEntryByWeekRow) => (
            <Fragment key={week.week}>
              <tr key={`week-${week.week}`} className="hover:bg-zinc-800">
                ...
                <td className="px-4 py-2 border border-white text-gray-200">
                  {week.daily_sessions.length}
                </td>
                <td className="px-4 py-2 border border-white text-gray-200">
                  {hoursFormatter(week.week_total_hours)}
                </td>
                <td className="px-4 py-2 border border-white text-gray-200 text-center">
                  {expandedWeek === week.week && (
                    <button
                      onClick={() => toggleExpandedWeek(week.week)}
                  ...
                  {week.daily_sessions.map((dailySessions) => (
                    <Fragment key={dailySessions.day}>
                      <tr className="bg-zinc-800">
                        <td
                          colSpan={2}
                          className="text-center px-4 py-2 border border-white text-gray-200"
                        >
                          {dailySessions.day}
                        </td>
                        <td className="px-4 py-2 border border-white text-gray-200">
                          {dailySessions.total_entries}
                        </td>
                        <td className="px-4 py-2 border border-white text-gray-200">
                          {hoursFormatter(dailySessions.total_hours)}
                        </td>
                        <td className="text-center px-4 py-2 border border-white text-gray-200">
                          {expandedDay === String(dailySessions.day) ? (
                            <button
                              onClick={() =>
                                toggleExpandedDay(String(dailySessions.day))
                              }
                              className="bg-purple-900 hover:bg-zinc-400 active:bg-purple-300 border-gray-200 border-2 text-gray-200 font-bold py-2 px-4 rounded"
                            >
                              Hide
                      ...
                      {expandedDay === String(dailySessions.day) && (
                        <>
                          ...
                          {dailySessions.sessions.map((session: Entry) => (
                            <tr
                            ...
                            >
                              ...
                              <td
                                colSpan={2}
                                className="px-4 py-2 border border-white text-gray-200"
                              >
                                {session.note}
                              </td>
                            </tr>
