...
export default function MonthlyHistoryTable({
  entries,
}: {
  entries: ParsedEntryByMonthRow[];
}) {
  ...
  return (
    <div className="border rounded-lg overflow-hidden">
      <table className="min-w-full text-white border border-white">
        ...
        <tbody>
          {entries?.map((month: ParsedEntryByMonthRow) => (
            <Fragment key={month.month}>
              <tr className=" hover:bg-zinc-800">
                ...
                <td className="px-4 py-2 border border-white text-gray-200">
                  {hoursFormatter(month.month_total_hours)}
                </td>
                <td className="text-center px-4 py-2 border border-white text-gray-200">
                  <button
                    onClick={() =>
                      toggle(setExpandedMonth, expandedMonth, month.month)
                    }
              ...
              {expandedMonth === month.month && (
                <>
                  ...
                  {month.weekly_sessions.map((week: ParsedEntryByWeekRow) => (
                    <Fragment key={week.week}>
                      <tr className="bg-zinc-800">
                        ...
                        >
                          {week.week} (Start: {week.week_start})
                        </td>
                        <td className="px-4 py-2 border border-white text-gray-200">
                        ...
                        <td className="text-center px-4 py-2 border border-white text-gray-200">
                          <button
                            onClick={() =>
                              toggle(setExpandedWeek, expandedWeek, week.week)
                            }
                            className={`border-2 font-bold py-2 px-4 rounded ${
                      ...
                      {expandedWeek === week.week && (
                        <>
                          ...
                          {week.daily_sessions.map((day: DaySession) => (
                            <Fragment key={day.day}>
                              <tr className="bg-zinc-700">
                                ...
                                <td className="px-4 py-2 border border-white text-gray-200">
                                  {hoursFormatter(day.total_hours)}
                                </td>
                                <td className="text-center px-4 py-2 border border-white text-gray-200">
                                  <button
                                    onClick={() =>
                              ...
                              {expandedDay === day.day && (
                                <>
                                  ...
                                  {day.sessions.map((session: Entry) => (
                                    <tr
                                      key={session.id}
                                      className="bg-purple-800 shadow-inner hover:bg-purple-700 text-center"
                                    >
                                      <td className="px-4 py-2 border border-white text-gray-200">
                                        {new Date(
                                          session.check_in
