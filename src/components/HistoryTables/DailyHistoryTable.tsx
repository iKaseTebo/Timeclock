...
import { DaySession, Entry } from "@/types/entries";
...
export default function DailyHistoryTable({
  ...
  const toggleExpandedRow = (key: string) => {
    setExpandedRow(key === expandedRow ? "" : key);
  };
  ...
  return (
    <div className="border rounded-lg overflow-hidden">
      <table className="min-w-full text-white border border-white">
        <thead>
          <tr>
            <th colSpan={2} className="px-4 py-2 border border-white">
              Date
            </th>
        ...
        <tbody>
          {entries?.map((day: DaySession) => (
            <Fragment key={day.day}>
              <tr key={`day${day.day}`} className="hover:bg-zinc-800">
                <td
                  colSpan={2}
                  className="px-4 border border-white text-gray-200"
                >
                  {day.day}
                </td>
                ...
                <td className="px-4 py-2 border border-white text-gray-200 text-center">
                  {expandedRow === day.day && (
                    <button
                      onClick={() => toggleExpandedRow(day.day)}
